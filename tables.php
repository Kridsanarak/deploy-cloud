...
if (!isset($_SESSION['role_id'])) {
    echo "<script>alert('ไม่สามารถระบุบทบาทของผู้ใช้ได้'); window.location.href = 'index.php';</script>";
    exit;
}
...
// ดึงข้อมูลงานในช่วง 7 วันถัดไป
$today = date('Y-m-d');
$nextWeek = date('Y-m-d', strtotime('+7 days'));

$sql = "SELECT 
    t.task_id, t.start_date, t.end_date, t.user_id, t.floor_id, 
    t.room_id, t.status_id, t.toilet_status_id, t.image, 
    u.fullname AS user_fullname, r.room_name
FROM task t
...
echo '</form>';
        ...
        echo '<div class="modal-header"><h5 class="modal-title" id="imageModalLabel' . $row["task_id"] . '">Image Preview</h5>';
    ...
    echo "<div class='card-body'><p class='text-center'>You have no tasks scheduled for the next 7 days.</p></div>";
}
